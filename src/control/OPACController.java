    ...
    private void handledBtnBuscarCodBarras(ActionEvent event) {
        ...
        if (!codBarrasOpacTxt.getText().isEmpty()) {
            ...
            if (!nombreEntidad.isEmpty()) {
                panelOpac.getChildren().clear();
                FichaTecnicaPanel crs = new FichaTecnicaPanel();
                panelOpac.add(crs.cargarFichaTecnica(nombreEntidad, codBarrasOpacTxt.getText()),0,0);
            } else {
                alert.showAlert("Anuncio", "Busqueda", "El codigo de barras no concuerda con ningun recurso");
            }
        } else {
