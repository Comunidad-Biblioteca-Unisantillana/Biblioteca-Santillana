...
public class MultaRevistaProfFab implements IMulta {
    ...
    public void actualizarDiasAtrasadosMulta() {
        try {
            List<MultaRevistaProf> listaMultas = readMultasRevistasProfDAO();
            for (int i = 0; i < listaMultas.size(); i++) {
                if (listaMultas.get(i).getEstadoCancelacion().equalsIgnoreCase("no")) {
                    MultaRevistaDAOProf multaDAO = new MultaRevistaDAOProf();
                    Date fechaDevolucion = getFechaDevolucionPrestamo(listaMultas.get(i).getCodPrestamoRevistaProf());
    ...
    public void generarMulta() {
        java.util.Date fechaActual = new java.util.Date();
        List<PrestamoRevistaProf> listaPres = readPrestamosRevistasProfDAO();
        List<DevolucionRevistaProf> listaDev = readDevolucionRevistasProfDAO();
        boolean existePrestamoDev = false;

        if (listaPres != null && listaDev != null) {
            for (int i = 0; i < listaPres.size(); i++) {
                int codPrestamo = listaPres.get(i).getCodPrestamoRevistaProf();
                ...
                if (fechaActual.after(listaPres.get(i).getFechaDevolucion())) {
                    for (int j = 0; j < listaDev.size(); j++) {
                        ...
                        if (Objects.equals(listaDev.get(j).getCodPrestamoRevistaProf(), codPrestamo)) {
                        ...
                        try {
                            ...
                            if (listaMultas != null) {
                                for (int k = 0; k < listaMultas.size(); k++) {
                                    if (listaMultas.get(k).getCodPrestamoRevistaProf()== codPrestamo) {
                                        existeMulta = true;
                            ...
                            if (!existeMulta) {
                                ...
                                MultaRevistaDAOProf multa = new MultaRevistaDAOProf();
                                ...
                                if (precioMulta != null) {
                                    MultaRevistaProf multaEst = new MultaRevistaProf(codPrestamo, diasAtrasados, 1, precioMulta.getValorpordia(), "no","no aplica",new Date(fechaActual.getTime()));
    ...
    private Date getFechaDevolucionPrestamo(int codigoPrestamo) {
        List<PrestamoRevistaProf> listaPrestamos = readPrestamosRevistasProfDAO();
        if (listaPrestamos != null) {
            for (int i = 0; i < listaPrestamos.size(); i++) {
                if (listaPrestamos.get(i) != null) {
                    if (listaPrestamos.get(i).getCodPrestamoRevistaProf()== codigoPrestamo) {
    ...
    private List<DevolucionRevistaProf> readDevolucionRevistasProfDAO() {
        DevolucionRevistaDAOProf dev = new DevolucionRevistaDAOProf();
        List<DevolucionRevistaProf> listaDev = null;
        try {
    ...
    private List<MultaRevistaProf> readMultasRevistasProfDAO() {
        MultaRevistaDAOProf multa = new MultaRevistaDAOProf();
        List<MultaRevistaProf> listaMulta = null;
        try {
    ...
    private List<PrestamoRevistaProf> readPrestamosRevistasProfDAO() {
        PrestamoRevistaDAOProf  pres = new PrestamoRevistaDAOProf();
        List<PrestamoRevistaProf> listaPrestamos = null;
        try {
