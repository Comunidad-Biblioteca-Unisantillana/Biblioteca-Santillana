...
public class PrestamoDiccionarioEstFabTest {
	...
	private void readAndDeletePrestamoTest(PrestamoDiccionarioEstFab instance) {
		if (instance != null) {
			try {
				...
				DiccionarioJpaController libJPA = new DiccionarioJpaController();
				Diccionario lib = libJPA.findDiccionario(prestamo.get(prestamo.size() - 1).getCodBarraDiccionario());
				...
				System.out.println("codigo de barra: " + lib.getCodbarradiccionario());
	...
	public void testRecursoNoDisponible() {
		...
		PrestamoDiccionarioEstFab instance = new PrestamoDiccionarioEstFab();
		DiccionarioJpaController controlLib = new DiccionarioJpaController();
		List<Diccionario> lib = controlLib.findDiccionarioEntities();
		boolean result = false;
		for (int i = 0; i < lib.size(); i++) {
			if (!lib.get(i).getDisponibilidad().equalsIgnoreCase("disponible")) {
				try {
					result = instance.ejecutarPrestamo(lib.get(i).getCodbarradiccionario(), "1760156", "1102515566");
					System.out.println("El prestamo se realizo");
					break;
				} catch (Exception e) {
				} catch (NoClassDefFoundError | ExceptionInInitializerError ex) {
	...
	public void testCrearPrestamoRecursos() {
		...
		for (int i = 0; i < lib.size(); i++) {
			if (lib.get(i).getDisponibilidad().equalsIgnoreCase("disponible")) {
				...
				System.out.println("|\n|");
				System.out.println("El libro con codigo de barra " + lib.get(i).getCodbarradiccionario()
